...
class MMovieItem extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isAllowDelete =
        ref.read(ticketsProvider.notifier).isAllowDeleteMovie(ref, movie.id!);

    return GestureDetector(
      onLongPress: !movie.active! && isAllowDelete
          ? () {
              FunctionUtil.confirmDelete(
                  onPressedConfirm: () {
                    ref.read(moviesProvider.notifier).remove(movie.id!);
                  },
                  content: 'Phim?');
            }
          : () {
